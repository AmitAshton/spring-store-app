...
/**
 * this class includes all the logic for the item log services,
 * like logging a new item, getting transacted items and more...
 */
@Service
...
public class ItemLogService {
    ...
    public List<ItemsTransactionAggregation> getTransactedItems(Optional<Long> quantity) {
        ...
        return getAggregationFromResult(result, "items")
                .getBuckets()
                .stream()
                .map(x -> parseAggregationCountAsObject("transactionAction", x.getAggregations(), x.getKeyAsString(), ItemsTransactionAggregation.class)).collect(Collectors.toList());
    }
