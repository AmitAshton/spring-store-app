...
/**
 * this class represents the controller for the shop and all the CRUD options for the shop
 */
@RestController
...
public class ShopController {
    ...
    @PostMapping
    public ResponseEntity<EntityModel<ShopDto>> createShop(@Valid @RequestBody ShopDto shopDto) throws ShopException {
    ...
    @PutMapping
    public ResponseEntity<EntityModel<ShopDto>> updateShop(@Valid @RequestBody ShopDto shopDto) {
    ...
    public CollectionModel<EntityModel<ShopDto>> getAllShops() {
        List<ShopDto> fetchedShops = shopService.shops().stream().map(shopMapper::shopToShopDto).collect(Collectors.toList());
